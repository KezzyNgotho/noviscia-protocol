...
| Token registration + `X-Noviscia-App-Token` credentials | API reference (`/developer/api`) documents token auth; indexer enforces token-scoped authz (`INDEXER_AUTHZ=strict`) | **implemented** |
