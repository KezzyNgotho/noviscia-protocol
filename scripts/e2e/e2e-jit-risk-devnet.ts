 ...
 *
 *  1. initialize_marketplace (admin) — MarketplaceState + USDC vault
 ...
 *  4. rent_slice  $10k @ δ=4e-9 → premium exactly $0.00004 (atomic, same-slot)
 *  5. reserve_slice $10k @ δ=4e-9, collateral C=$11k (110% haircut)
 *  6. settle_slice profit  → full C released back to the desk
 *  7. reserve_slice $10k + settle adversarial −$13k
 *        → C forfeited ($11k), residual $2k → pool absorbs min(10%·A, residual) = $1k
 *        → loss ratio >> 8000bps → marketplace AUTO-FREEZES
 *  8. unfreeze (admin), sweep_premiums → 85/15 NAV/backstop split
 *  9. suspend desk_B → further reserve_slice REJECTED
...
async function reserveAndSettle(
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      await jr.reserveSlice(mm.publicKey, amount, delta, slot, mm).rpc();
    } catch (e: any) {
    ...
    try {
      await jr.settleSlice(mm.publicKey, slot, pnl).rpc();
      return;
    } catch (e: any) {
      const msg = e?.message ?? String(e);
      if (/ReceiptExpired|stale|aged/i.test(msg)) {
        ...
        await jr.reapExpiredSlice(mm.publicKey, slot).rpc();
      ...
      if (devnetHiccup(msg)) {
        ...
        if (await conn.getAccountInfo(receiptPda)) {
          await jr.reapExpiredSlice(mm.publicKey, slot).rpc().catch(() => {});
        }
...
async function main() {
  ...
  const idl = JSON.parse(fs.readFileSync(path.join(ROOT, 'target/idl/jit_risk.json'), 'utf-8'));
  ...
  // Desks need ~0.0015 SOL each: reserve_slice pays rent for the receipt PDA
  ...
  if (!(await conn.getAccountInfo(marketplacePda))) {
    await jr.initializeMarketplace({
  ...
  console.log('✓ initialize_marketplace');
  ...
  if (!(await conn.getAccountInfo(jr.mmRegistration(deskA.publicKey)))) {
    await jr.registerMm(deskA.publicKey, usd(20_000)).rpc();
  }
  if (!(await conn.getAccountInfo(jr.mmRegistration(deskB.publicKey)))) {
    await jr.registerMm(deskB.publicKey, usd(20_000)).rpc();
  }
  console.log('✓ register_mm deskA + deskB (ceiling $20k each)');

  // ── 4. rent_slice (atomic, same-slot zero in-flight window) ────────────────
  ...
  await jr.rentSlice(deskA.publicKey, usd(10_000), delta, deskA).rpc();
  ...
  assert(Number(postRent.value.amount) === Number(preRent.value.amount) + rentPremium,
    'rent_slice premium must land in vault ($0.00004)');
  ...
  console.log(`✓ rent_slice: $10k @ δ=4e-9 → premium ${rentPremium} raw ($0.00004)`, `atomic_sessions=${Number(mp.atomicSessions)}`);

  // Devnet RPC lag ⇒ widen the in-flight window to 16 slots (program caps at 16).
  await jr.updateParams({ maxSlotAge: 16 }).rpc();
  ...
  try {
    await jr.rentSlice(deskA.publicKey, usd(1_000), delta, deskA).rpc();
  } catch (e: any) {
    rentBlocked = /Frozen/i.test(e?.message ?? String(e));
    if (!rentBlocked) throw e;
  }
  assert(rentBlocked, 'rent_slice must be rejected while frozen');
  console.log('✓ rent_slice rejected while frozen');

  // ── 8. unfreeze + sweep 85/15 ──────────────────────────────────────────────
  await jr.unfreeze().rpc();
  mp = await jr.getMarketplace();
  ...
  if (pending > 0) {
    ...
    console.log(`✓ sweep_premiums: ${pending} raw → NAV ${pending - expected} / backstop ${expected}`);
  } else {
  ...
  await jr.suspendMm(deskB.publicKey).rpc();
  let suspendBlocked = false;
  try {
    const s = await conn.getSlot();
    await jr.reserveSlice(deskB.publicKey, usd(1_000), delta, s, deskB).rpc();
  } catch (e: any) {
  ...
  console.log('✓ suspend_mm deskB → reserve_slice rejected');
