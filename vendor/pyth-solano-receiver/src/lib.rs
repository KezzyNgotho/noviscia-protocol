...
//! Vendored Pyth Solana Receiver program.
//!
//! Vendored from upstream pyth-crosschain
//! `target_chains/solana/programs/pyth-solana-receiver` at commit
//! `4f7d3c5324659f3a24be13212e07f7574c51f17d` (Apache-2.0) and trimmed to the
//! surface this repository consumes:
//!
//!   * `post_update_atomic` — the real guardian-signature + VAA + merkle
//!     verification path that position-tracker CPIs into for JIT oracle
//!     updates. Kept verbatim (borrowed from Wormhole's
//!     `parse_and_verify_vaa/verify_encoded_vaa_v1.rs` + Pyth's
//!     `verify_guardian_signature` / `verify_merkle_proof`).
//!   * `initialize` + the governance handlers (data sources, fee, wormhole
//!     address, minimum signatures, two-step authority transfer).
//!   * `reclaim_rent` (price-update account rent reclaim).
//!
//! Trimmed away: `post_update`, `post_twap_update`, `reclaim_twap_rent`
//! (`post_update` needs `wormhole_core_bridge_solana::sdk::VaaAccount`.
//! `PostedVaaV1` / zero-copy VAA decoding — none used by position-tracker).
//! The verification core shared by those handlers is untouched.
...
use {
    ...
    wormhole_core_bridge_solana::{sdk::legacy::AccountVariant, state::GuardianSet},
