...
| 4 | Atomic Reversion Control | 🟢 Proven On Paper / Code | $0 Capital Leakage on failed transactions | sdk/src/waterfall.ts (paper proof) | Programs not deployed |
...
| 7 | Time-Accelerated Timeout Locks | 🟡 Simulated / Sandbox | 60s Expiry Execution (1440x scale) | — | Requires on-chain desk creation + time-advance to observe 24h/2h window behavior. The sandboxAccelerator.ts models this at 1440x acceleration. Live measurement: create desk on devnet, advance clock via warp-slot, observe when the facility freezes. |
